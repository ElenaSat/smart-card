          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Nombre Usuario</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              @for (item of items$ | async; track item.idUsuarioSistema) {
                <tr class="hover:bg-gray-50 transition-colors">
                  <td class="px-6 py-4 text-sm text-gray-900">{{ item.idUsuarioSistema }}</td>
                  <td class="px-6 py-4 text-sm text-gray-600">{{ item.nombreUsuario || '-' }}</td>
                  <td class="px-6 py-4 text-right text-sm font-medium">
                    <a [routerLink]="['edit', item.idUsuarioSistema]" class="text-indigo-600 hover:text-indigo-900 mr-4">Editar</a>
                    <button (click)="onDelete(item.idUsuarioSistema)" class="text-red-600 hover:text-red-900">Eliminar</button>
                  </td>
                </tr>
              } @empty {
                <tr><td colspan="3" class="px-6 py-12 text-center text-gray-500">No hay usuarios del sistema</td></tr>
              }
            </tbody>
          </table>
        </div>
