...
@Component({
    ...
    <div class="p-6 max-w-2xl mx-auto">
      <div class="mb-6">
        <a routerLink="/tipo-tarjetas" class="text-indigo-600 hover:text-indigo-800 flex items-center gap-2">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">{{ isEdit ? 'Editar Tipo' : 'Nuevo Tipo' }}</h1>
        ...
        @if (!(loading$ | async)) {
          <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-4">
            ...
            <div class="flex gap-4 pt-4">
              ...
              <a routerLink="/tipo-tarjetas" class="flex-1 text-center bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-3 rounded-lg font-medium transition-colors">
                Cancelar
              </a>
            </div>
          </form>
